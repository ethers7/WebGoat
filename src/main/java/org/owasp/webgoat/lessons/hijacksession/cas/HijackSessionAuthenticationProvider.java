...
// Lesson design: ids handed out by GENERATE_SESSION_ID are intentionally sequential, so the learner
// can spot the gap left by another login and hijack that session. Only this lesson's simulated
// session store below uses them; the starting offset comes from a SecureRandom, not a weak PRNG.
